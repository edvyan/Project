...
## 7. Efficient Online Summarization of Microblogging Streams (Tairo)
[Efficient Online Summarization of Microblogging Streams](https://aclanthology.org/E14-4046.pdf)

### Summary
The paper presents a novel approach to summarizing data generated on microblogging services. The proposed method, called Twitter Online Word Graph Summarizer (TOWGS), is an online abstractive summarization algorithm that processes data in a single pass, eliminating the need to save any input data and improving the running time. The algorithm uses a word graph and optimization techniques such as decaying windows and pruning. It outperforms the baseline in terms of summary quality, as well as time and memory efficiency.

### Methodology
TOWGS uses a word graph where each node is a bigram. The graph is built from trigrams, which significantly increases the number of nodes but also improves the quality of the summaries. To manage the increased memory and time requirements, the graph is pruned periodically. The graph is constantly updated with tweets and can generate summaries in real time using the latest information. The algorithm uses decaying windows to keep the results relevant to what is popular at query time.

### Experimental step
The algorithm was tested on a dataset of 3.4 million tweets. The baseline for comparison was an algorithm designed to summarize related tweets. A set of important events relative to the input data was determined and a group of judges rated the summaries generated by both algorithms for the given events.

### Results
TOWGS not only proved capable of online summarization, but it also outperformed the batch-based event-filtered baseline in terms of result quality. The algorithm was able to process a day of data from the corpus (around 485,000 tweets) in just under three minutes. The amount of RAM used by the algorithm was between 1.5 - 2 GB.

## 8. “Winter is here”: Summarizing Twitter Streams related to Pre-Scheduled Events (Tairo)
[“Winter is here”: Summarizing Twitter Streams related to Pre-Scheduled Events](https://aclanthology.org/W19-3412.pdf)

### Summary
This research proposes a method for summarizing highlights from tweet streams related to scheduled events (such as TV shows or sports games). Specifically, the algorithm is evaluated on tweets collected around the popular TV show “Game of Thrones.”

### Methodology
To identify highlights, the study plots the number of tweets for each episode and identifies peaks in tweet volume. Next, it identifies the characters most mentioned during these highlights. To summarize these highlights, it uses context2vec to create vector representations of tweets and applies K-Means clustering to these vectors.

### Experimental Steps
The algorithm is evaluated on tweets collected around episodes 3 and 4 of “Game of Thrones” Season 7. It compares the summaries generated by the proposed model with plot summaries obtained from the IMDB website.

### Results
The proposed model outperforms all five competing summarization algorithms. Additionally, examples of highlight summaries for episodes 3 and 4 are provided.
